...
def correlation_topk_edges(train_df: pd.DataFrame, top_k: int) -> pd.DataFrame:
    data = train_df.copy()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run correlation top-k neighbor sweep with matched random controls.")
    ...
    edge_audit_rows = []
    for k in ks:
        corr_edges = correlation_topk_edges(splits["train"], top_k=k)
        corr_graph = f"corr_top{k}_directed"
        validate_edges(corr_edges, basin_id_set, graph_type=corr_graph)
        corr_edges.to_csv(args.output_dir / f"edges_{corr_graph}.csv", index=False)
        edge_audit_rows.append({"graph_type": corr_graph, "k": k, "seed": np.nan, "n_edges": len(corr_edges)})
    ...
    test["control"] = np.where(test["graph_type"].str.startswith("corr_"), "corr", "random")
    summary_rows = []
    for (model_name, k), group in test.groupby(["model_name", "k"]):
        corr = group[group["control"].eq("corr")]
        ...
        row = {
            ...
            "k": int(k),
            "corr_rmse_cm": float(corr.iloc[0]["rmse_cm"]),
            "corr_mae_cm": float(corr.iloc[0]["mae_cm"]),
            "corr_pearson_r": float(corr.iloc[0]["pearson_r"]),
        }
        if not random.empty:
            ...
            row["corr_minus_random_mean_rmse_cm"] = row["corr_rmse_cm"] - row["random_mean_rmse_cm"]
            row["corr_beats_all_random_seeds"] = bool(row["corr_rmse_cm"] < row["random_min_rmse_cm"])
        summary_rows.append(row)
    summary = pd.DataFrame(summary_rows).sort_values(["model_name", "corr_rmse_cm"])
